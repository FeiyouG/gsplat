...
inline __device__ void persp_proj_vjp(
    ...
    if (length(Cinv_uvh.x, Cinv_uvh.y, Cinv_uvh.z) < 1E-12 || D == 0) {
        ...
        v_l = glm::dot(
              ...
              nl;
        vec2 v_plane = {
            factor * (-v_ray_normal_vector.x + v_ray_plane_uv.x),
        ...
        if (well_conditioned) {
            v_cov3d_ += -glm::outerProduct(
                Cinv_uvh,
                cov3d_inv_u_Cinv_u *
                    (uvh * (-tmp) + glm::transpose(nJ_inv_T) * v_plane)
            );
        } else {
            mat3 v_cov3d_inv = glm::outerProduct(
                uvh,
                (-tmp * uvh + glm::transpose(nJ_inv_T) * v_plane) /
                    u_Cinv_u_clmap
            );
            vec3 v_eigenvector_min =
                (v_cov3d_inv + glm::transpose(v_cov3d_inv)) * eigenvector_min;
            for (int j = 0; j < 3; j++) {
                if (j != min_id) {
                    float scale =
                        glm::dot(cov3d_eigen_vector[j], v_eigenvector_min) /
                        min(cov3d_eigen_value[min_id] - cov3d_eigen_value[j],
                            -0.0000001f);
                    v_cov3d_ += glm::outerProduct(
                        cov3d_eigen_vector[j] * scale, eigenvector_min
                    );
                }
            }
        }

        vec3 v_uvh = cov3d_inv_u_Cinv_u *
                     (2 * (-tmp) * uvh + glm::transpose(nJ_inv_T) * v_plane);
        mat3x2 v_nJ_inv_T = glm::outerProduct(v_plane, Cinv_uvh_u_Cinv_u);

        // derivative of u v in factor, uvh and nJ_inv_T variables.
        v_u = v_nl * 2 * u // dnl/du
              + v_uvh.x + (v_nJ_inv_T[0][1] + v_nJ_inv_T[1][0]) * (-v) +
              2 * v_nJ_inv_T[1][1] * u - v_nJ_inv_T[2][0];
        v_v = v_nl * 2 * v // dnl/du
              + v_uvh.y + (v_nJ_inv_T[0][1] + v_nJ_inv_T[1][0]) * (-u) +
              2 * v_nJ_inv_T[0][0] * v - v_nJ_inv_T[2][1];
    }
    ...
    // fov clipping
    float l3 = l * l * l;
    float v_mean3d_x = -fx * rz2 * v_J[2][0] + v_u * rz - v_nJ_T[0][2] * rz2 +
                       v_nJ_T[2][0] * (1 / l - tx * tx / l3) +
                       (v_nJ_T[2][1] * ty + v_nJ_T[2][2] * z) * (-tx / l3) +
                       v_l * tx / l;
    ...
    v_mean3d_.z += -fx * rz2 * v_J[0][0] - fy * rz2 * v_J[1][1] +
                   2.f * fx * tx * rz3 * v_J[2][0] +
                   2.f * fy * ty * rz3 * v_J[2][1] -
                   (v_u * tx + v_v * ty) * rz2 + v_nJ_T[0][0] * (-rz2) +
                   v_nJ_T[1][1] * (-rz2) + v_nJ_T[0][2] * (2 * tx * rz3) +
                   v_nJ_T[1][2] * (2 * ty * rz3) +
                   (v_nJ_T[2][0] * tx + v_nJ_T[2][1] * ty) * (-z / l3) +
                   v_nJ_T[2][2] * (1 / l - z * z / l3) + v_l * z / l;
    v_mean3d += v_mean3d_;
}
