...
__global__ void rasterize_to_pixels_3dgs_fwd_kernel(
    ...
    const scalar_t *__restrict__ ray_ts,      // [I, N] or [nnz]
    const vec2 *__restrict__ ray_planes,      // [I, N, 2] or [nnz, 2]
    const vec3 *__restrict__ normals,         // [I, N, 3] or [nnz, 3]
    const scalar_t *__restrict__ backgrounds, // [I, CDIM]
    const bool *__restrict__ masks,           // [I, tile_height, tile_width]
    const scalar_t *__restrict__ Ks,          // [I, 9]
...
#pragma unroll
    ...
    //  index of the Gaussian that makes opacity cross 0.5 (Rade-GS)
