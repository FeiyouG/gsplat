...
__global__ void rasterize_to_pixels_3dgs_bwd_kernel(
    ...
    const scalar_t *__restrict__ ray_ts,       // [nnz]
    const vec2 *__restrict__ ray_planes,   // [nnz, 2]
    const vec3 *__restrict__ normals,      // [nnz, 3]
    ...
    const scalar_t *__restrict__ v_render_expected_depths, // [..., image_height, image_width, 1]
    const scalar_t *__restrict__ v_render_median_depths, // [..., image_height, image_width, 1]
    const vec3 *__restrict__ v_render_expected_normals, // [..., image_height, image_width, 3]
    // grad inputs
...
void launch_rasterize_to_pixels_3dgs_bwd_kernel(
    ...
    int64_t shmem_size =
        tile_size * tile_size *
        (sizeof(int32_t) + sizeof(vec3) + sizeof(vec3) + sizeof(float) * CDIM + sizeof(float) + sizeof(vec2) + sizeof(vec3));
